...
def parse_args():
    p = argparse.ArgumentParser(description="Train Method2 decoder: article+plan -> summary")
    ...
    p.add_argument("--output-dir", default="outputs/models/method2_decoder")
...
def main():
    ...
    def preprocess(batch):
        plans = [build_plan_from_reference(h, max_points=3) for h in batch["highlights"]]
        sources = [
            "Generate a factual 3-4 sentence summary from article and plan.\n\n"
            f"Article: {a}\n\nPlan:\n{p}\n\nSummary:"
            for a, p in zip(batch["article"], plans)
        ]
    ...
    print(f"Saved decoder model to {args.output_dir}")
